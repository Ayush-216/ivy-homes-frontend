      ...
      <section className="bg-[#0a0a0a] p-6 rounded-2xl border border-gray-800 shadow-[0_0_15px_rgba(6,182,212,0.05)]">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <div className="relative">
            ...
            <input type="text" placeholder="Search property name..." value={searchName} onChange={(e) => setSearchName(e.target.value)} className="w-full bg-gray-950 border border-gray-700 text-white pl-10 pr-4 py-2.5 rounded-xl focus:border-cyan-500 focus:shadow-[0_0_10px_rgba(6,182,212,0.3)] outline-none transition-all" />
          </div>
          <div className="relative">
            <MapPin className="absolute left-3 top-3 text-gray-500 w-5 h-5" />
            <input type="text" placeholder="Filter Locality..." value={filterLocality} onChange={(e) => setFilterLocality(e.target.value)} className="w-full bg-gray-950 border border-gray-700 text-white pl-10 pr-4 py-2.5 rounded-xl focus:border-cyan-500 focus:shadow-[0_0_10px_rgba(6,182,212,0.3)] outline-none transition-all" />
          </div>
          <div className="relative">
            <Bed className="absolute left-3 top-3 text-gray-500 w-5 h-5" />
            <select value={filterBhk} onChange={(e) => setFilterBhk(e.target.value)} className="w-full bg-gray-950 border border-gray-700 text-white pl-10 pr-4 py-2.5 rounded-xl focus:border-cyan-500 focus:shadow-[0_0_10px_rgba(6,182,212,0.3)] outline-none appearance-none transition-all">
        ...
        <div className="border-t border-gray-800 pt-6 mb-6">
          ...
          <div className="flex flex-col md:flex-row gap-6 items-center">
            <div className="flex-1 w-full relative h-2 bg-gray-800 rounded-full">
               <input type="range" min="0" max="100000000" step="100000" value={minPrice} onChange={(e) => setMinPrice(Math.min(Number(e.target.value), maxPrice - 100000))} className="absolute w-full top-0 appearance-none bg-transparent pointer-events-auto h-2 z-20 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-cyan-400 [&::-webkit-slider-thumb]:shadow-[0_0_10px_rgba(6,182,212,0.8)] cursor-pointer" />
               <input type="range" min="0" max="100000000" step="100000" value={maxPrice} onChange={(e) => setMaxPrice(Math.max(Number(e.target.value), minPrice + 100000))} className="absolute w-full top-0 appearance-none bg-transparent pointer-events-auto h-2 z-30 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-blue-500 [&::-webkit-slider-thumb]:shadow-[0_0_10px_rgba(59,130,246,0.8)] cursor-pointer" />
               <div className="absolute h-full bg-gradient-to-r from-cyan-500 to-blue-600 rounded-full z-10 shadow-[0_0_8px_rgba(6,182,212,0.5)]" style={{ left: `${(minPrice / 100000000) * 100}%`, right: `${100 - (maxPrice / 100000000) * 100}%` }} />
            </div>
            <div className="flex items-center gap-4 w-full md:w-auto">
              <div className="relative">
                <IndianRupee className="absolute left-3 top-2.5 text-gray-500 w-4 h-4" />
                <input type="number" value={minPrice} onChange={(e) => setMinPrice(Number(e.target.value))} className="w-32 bg-gray-950 border border-gray-700 text-white pl-8 pr-2 py-2 rounded-lg text-sm focus:border-cyan-500 focus:shadow-[0_0_10px_rgba(6,182,212,0.3)] outline-none transition-all" />
              </div>
              <span className="text-gray-600">to</span>
              <div className="relative">
                <IndianRupee className="absolute left-3 top-2.5 text-gray-500 w-4 h-4" />
                <input type="number" value={maxPrice} onChange={(e) => setMaxPrice(Number(e.target.value))} className="w-32 bg-gray-950 border border-gray-700 text-white pl-8 pr-2 py-2 rounded-lg text-sm focus:border-cyan-500 focus:shadow-[0_0_10px_rgba(6,182,212,0.3)] outline-none transition-all" />
              </div>
            </div>
          </div>
        </div>
        ...
        <button onClick={() => setShowAdvanced(!showAdvanced)} className="flex items-center gap-2 text-cyan-400 text-sm font-semibold hover:text-cyan-300 hover:drop-shadow-[0_0_8px_rgba(6,182,212,0.8)] transition-all">
        ...
        {showAdvanced && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4 pt-4 border-t border-gray-800">
            <select value={filterPropType} onChange={(e) => setFilterPropType(e.target.value)} className="bg-gray-950 border border-gray-700 text-white px-4 py-2 rounded-lg focus:border-cyan-500 focus:shadow-[0_0_10px_rgba(6,182,212,0.3)] outline-none transition-all">
              <option value="">Property Type</option>
              <option value="apartment">Apartment</option>
              <option value="villa">Villa</option>
              <option value="independent house">Independent House</option>
            </select>
            <select value={filterBath} onChange={(e) => setFilterBath(e.target.value)} className="bg-gray-950 border border-gray-700 text-white px-4 py-2 rounded-lg focus:border-cyan-500 focus:shadow-[0_0_10px_rgba(6,182,212,0.3)] outline-none transition-all">
            ...
            <select value={filterFacing} onChange={(e) => setFilterFacing(e.target.value)} className="bg-gray-950 border border-gray-700 text-white px-4 py-2 rounded-lg focus:border-cyan-500 focus:shadow-[0_0_10px_rgba(6,182,212,0.3)] outline-none transition-all">
            ...
            <select value={filterPostedBy} onChange={(e) => setFilterPostedBy(e.target.value)} className="bg-gray-950 border border-gray-700 text-white px-4 py-2 rounded-lg focus:border-cyan-500 focus:shadow-[0_0_10px_rgba(6,182,212,0.3)] outline-none transition-all">
